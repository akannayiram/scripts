...
@REM  
@REM ######################################################
@REM    ClearCachePSDomains.cmd 
@REM      - To clear cache of App, Prcs, and Web (Any number of Domains)
@REM      - Requires 1 parameter: Properties file containing Domain details
@REM   Usage:
@REM      ClearCachePSDomains.cmd <domains.properties>
@REM ######################################################
@REM
@Echo off
...
if not exist %PS_CFG_HOME%\NUL (
   set msg=[%date% %time%] - ERROR. PS_CFG_HOME [%PS_CFG_HOME%] does not exist and Aborting the script [%0]
   echo !msg! >> %LOGFILE%
   echo !msg! >> %ERRFILE%
   goto :fin
)
if defined %APPDOMAINS% (
   for %%a in ("%APPDOMAINS:,=" "%") do (
       SET APPDOMJOLT=%%~a
       for /f "tokens=1,2 delims=:" %%A in ("!APPDOMJOLT!") do (
           set APPDOM=%%A
           set JOLT=%%B
